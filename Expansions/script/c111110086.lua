-- Eraser's Waifu (Versión 2.0)
local s,id=GetID()
function s.initial_effect(c)
	aux.AddCodeList(c,21208154,62180201,57793869)
	
	-- (1) Invocación Especial desde Mano (Por Destrucción)
	...
	-- (3) Invocación Normal en Turno Rival
	...
	c:RegisterEffect(e4_bsh)
end

function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return rp==1-tp and eg:IsExists(Card.IsControler,1,nil,tp)
end
...
-- FILTROS EFECTO 2 (ESTILO DIVINE COMEBACK)
...
function s.thfilter2(c)
	return aux.IsCodeListed(c,57793869) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand()
end
