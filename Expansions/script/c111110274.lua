...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	-- ● Efecto Ganado: Cambiar a defensa y proteger otros monstruos de ataques
...
function s.slifertg(e,tp,eg,ep,ev,re,r,rp,chk)
	-- Puede activarse si Slifer puede cambiar a posición de defensa
...
function s.sliferop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	-- Cambiar a posición de defensa en resolución
	if c:IsRelateToEffect(e) and c:IsAttackPos() and Duel.ChangePosition(c,POS_FACEUP_DEFENSE)>0 then
		-- Si cambia con éxito, el oponente no puede seleccionar otros monstruos para ataques este turno
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_FIELD)
		e1:SetCode(EFFECT_CANNOT_SELECT_BATTLE_TARGET)
		e1:SetTargetRange(0,LOCATION_MZONE)
		e1:SetValue(function(e,tc) return tc~=c end) -- El oponente solo puede atacar a Slifer
