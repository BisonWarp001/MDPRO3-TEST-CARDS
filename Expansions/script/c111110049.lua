...
function s.negop(e,tp,eg,ep,ev,re,r,rp)
	-- 1. Negamos la activación
	if Duel.NegateActivation(ev) then
		-- 2. Buscamos la carta física que activó el efecto (re:GetHandler)
		local rc=re:GetHandler()
		-- 3. Si la carta sigue ahí, la desterramos
		if rc:IsRelateToEffect(re) and Duel.Remove(rc,POS_FACEUP,REASON_EFFECT)~=0 then
			-- 4. Chequeo de Ra: Buscamos el ID 10000010 en tu campo
			-- Usamos esta forma para que la inmunidad de Ra no "esconda" su presencia
