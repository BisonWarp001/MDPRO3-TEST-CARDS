--Royal Guardian of the Chosen Pharaoh
local s,id=GetID()
...
-- Lógica (1)
-- Filtro para el efecto (1): "The Chosen Pharaoh" o M/T de soporte en GY
function s.thfilter(c)
	return (c:IsCode(111110256) or (c:IsType(TYPE_SPELL+TYPE_TRAP) and c:ListsCode(10000000, 10000010, 10000020)))
		and c:IsAbleToHand()
end

-- Lógica (1): Reveal -> Add from GY -> Special Summon
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then 
		return not c:IsPublic() -- Costo implícito de revelar
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	-- Revelar la carta en mano
	if not c:IsRelateToEffect(e) then return end
	Duel.ConfirmCards(1-tp,c)
	
	-- 1. Añadir del Cementerio a la Mano
	...
	if #g>0 and Duel.SendtoHand(g,nil,REASON_EFFECT)>0 then
		...
		-- 2. "And if you do, Special Summon this card"
		Duel.BreakEffect()
